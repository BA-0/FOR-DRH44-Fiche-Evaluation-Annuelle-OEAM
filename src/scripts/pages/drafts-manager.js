...
async function loadDrafts() {
    ...
    try {
        ...
        let drafts = result.evaluations || result || [];
        
        // Vérifier que c'est bien un tableau
        if (!Array.isArray(drafts)) {
            console.error('Les données reçues ne sont pas un tableau:', drafts);
            drafts = [];
        }
        
        // Filter only drafts
        drafts = drafts.filter(evaluation => evaluation.status === 'draft' || evaluation.statut === 'draft');
